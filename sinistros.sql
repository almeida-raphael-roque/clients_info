...
from viavante.insurance_dam_event ide
	...
	left outer join viavante.insurance_dam_event_budget ideb on ideb.parent = idr.id
	...
	left outer join (
		select distinct iv.board as "placa",
		 ...
		 tv.descricao  as tipo_veiculo,
		 tca.descricao as "tipo_carroceria"
		from viavante.insurance_vehicle iv
			left outer join viavante.marca_veiculo mar on mar.codigo = iv.code_brand_vehicle
			left outer join viavante.modelo_veiculo mv on mv.codigo = iv.code_model_vehicle
			left outer join viavante.tipo_carroceria tca on tca.codigo = iv.body_type
	...
	left outer join (
		select distinct it.board as placa,
			...
			it.brand as modelo,
			tca.descricao as tipo_carroceria
		from viavante.insurance_trailer it
			left join viavante.tipo_carroceria tca on tca.codigo = it.body_type
		where it.board <> 'SEM-PLACA'
	) as placa_reboque on placa_reboque.placa = idr.board 
	...
	left outer join (
		...
		from viavante.insurance_dam_event_budget ideb
			left outer join viavante.insurance_dam_event_reference idr on idr.id = ideb.parent
			left outer join viavante.insurance_dam_event ide on ide.id = idr.parent
	...
	left outer join (
		...
		from viavante.insurance_dam_event_budget
		group by parent,
			code_material
	) as vlorc_cas on vlorc_cas.parent = idr.id -- subquery para coletar os beneficios que foram acionados
	left outer join (
		...
		from viavante.insurance_assistance_benefits iab
			left outer join viavante.insurance_reg_set_coverage irsc on irsc.id = iab.coverage
...
from tag.insurance_dam_event ide
	...
	left outer join tag.insurance_dam_event_budget ideb on ideb.parent = idr.id
	...
	left outer join (
		select distinct iv.board as "placa",
		 ...
		 tv.descricao  as tipo_veiculo,
		 tca.descricao as "tipo_carroceria"
		from tag.insurance_vehicle iv
			left outer join tag.marca_veiculo mar on mar.codigo = iv.code_brand_vehicle
			left outer join tag.modelo_veiculo mv on mv.codigo = iv.code_model_vehicle
			left outer join tag.tipo_carroceria tca on tca.codigo = iv.body_type
	...
	left outer join (
		select distinct it.board as placa,
			...
			it.brand as modelo,
			tca.descricao as tipo_carroceria
		from tag.insurance_trailer it
			left join tag.tipo_carroceria tca on tca.codigo = it.body_type
		where it.board <> 'SEM-PLACA'
	) as placa_reboque on placa_reboque.placa = idr.board 
	...
	left outer join (
		...
		from tag.insurance_dam_event_budget ideb
			left outer join tag.insurance_dam_event_reference idr on idr.id = ideb.parent
			left outer join tag.insurance_dam_event ide on ide.id = idr.parent
	...
	left outer join (
		...
		from tag.insurance_dam_event_budget
		group by parent,
			code_material
	) as vlorc_cas on vlorc_cas.parent = idr.id -- subquery para coletar os beneficios que foram acionados
	left outer join (
		...
		from tag.insurance_assistance_benefits iab
			left outer join tag.insurance_reg_set_coverage irsc on irsc.id = iab.coverage
	...
	where cast(ide.date_event as date) > DATE('2025-08-01')